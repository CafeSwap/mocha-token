...
contract MochaToken is Context, IBEP20, Ownable {
    ...
    */
    ...
    uint constant public BURN_FEE = 900; // 0.9
    uint constant public REWARD_FEE = 100; // 0.1
    uint constant public MAX_FEE = 100000;
    uint256 constant public HARD_CAP = 500000e18;
    ...
    modifier checkHardCap(uint256 amount) {
        require(totalSupply().add(amount) <= HARD_CAP, "Supply is greater than HARD_CAP");
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal   {
        ...
        if(whiteList[sender] == true || whiteList[recipient] == true){
            ...
            (uint256 toBurn, uint256 toReward, uint256 receiverAmount) = calculateFees(amount);

            // NOTE POTENTIAL BUG HERE
            _burn(sender, toBurn);
            _transferToken(sender, rewardAddress, toReward);
            _transferToken(sender, recipient, receiverAmount);
        }
    }
    ...
    function setRewardAddress(address _rewardAddress) public onlyOwner {
        require(_rewardAddress != address(0x00),"zero address");
        rewardAddress = _rewardAddress;
    ...
    function calculateFees(uint256 amount) public pure returns (uint256, uint256, uint256) {
        ...
        amount = amount.sub(toBurn+toReward);
        return (toBurn, toReward, amount);
    }
    ...
    function whiteListAccount(address account, bool status) external onlyAdmin() {
